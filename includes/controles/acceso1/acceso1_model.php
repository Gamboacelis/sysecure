...
function obtenerVisitantesAsignados() {
    ...
    if ($val->num_rows > 0) {
        while ($row = $val->fetch_object()) {
            ...
            $autorizado=(consultaVisitanteAutorizado($row->VIP_COD)=='1')?'<span class="label label-primary">Ingreso Autorizado: '.$fecha.'</span>':'<a class="btn btn-success" title="Permitir Acceso" href="javascript:permitirAccesoVisitante(' . $cadenaParametros . ')"><i class="fa fa-child"></i> Permitir Acceso</a>';
            $retval.='<tr>
                            <td>' . $row->VIP_COD . '</td>
...
function consultaVisitanteAutorizado($codVisita){
    ...
    $val = $dbmysql->query($sql);
    if($val->num_rows>0){return 1;}else{return 0;}
}
