...
--
-- Servidor: localhost
-- Tiempo de generación: 02-09-2014 a las 20:03:19
...
(1, 1, 2, 'Horario en la Mañana', 'Martes', '08:20:00', '20:20:00', 'A'),
(2, 1, 3, 'Horario en la Tarde', 'Sábado', '14:00:00', '16:00:00', 'I'),
(3, 1, 1, 'Horaio en la Tarde', 'Domingo', '16:30:00', '18:30:00', 'A'),
(4, 1, 2, 'Visita Conyugal', 'Miércoles', '10:30:00', '12:00:00', 'A'),
...
(3, 1, 1, 'Carlos', 'Rosales', '1720090057', 'rcontreras.jpg', 'L'),
...
CREATE TABLE IF NOT EXISTS `sys_visitante` (
  ...
  PRIMARY KEY (`VIS_COD`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Almacen de Visitantes que pueden Acceder a un Centro' AUTO_INCREMENT=7 ;
...
CREATE TABLE IF NOT EXISTS `sys_visitante_ppl` (
  ...
  PRIMARY KEY (`VIP_COD`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COMMENT='Listado de Visitantes que se encuentran ligados a un PPL' AUTO_INCREMENT=6 ;
...
(1, NULL, 6, 1, 'Por cualquier cosa', NULL, NULL);
...
(4, 1, 'A', 2, 'aaaaaaaaa', 'ssssssssssss', '1720802394', 'cpazmino.jpg', 'A'),
(8, 1, 'A', 2, 'Willian', 'Uribe', '1720802394', '', 'A'),
(9, 1, 'A', 1, 'qqqqqqqqqq', 'qqqqqqqqqqqqqq', '1720802394', '', 'A'),
(10, 1, 'A', 2, 'Willian', 'Espinosa', '1720802394', '', 'A'),
(11, 1, 'A', 1, 'Carlos', 'Alfredo', 'sdfa', 'wie.jpg', 'A'),
(12, 1, 'A', 1, 'sx', 'Peralta', '1720802394', 'ovillamil.jpg', 'A'),
(13, 1, 'A', 0, 'Administrador', 'Peralta', '1720802394', 'ambiente23.jpg', 'A'),
(14, 2, 'B', 0, 'Andres', 'Hernandez', '1720802394', 'dvasquez.jpg', 'A');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `bulkenco_sysecure`.`sys_vw_control3` AS select `c`.`CON_COD` AS `CON_COD`,`c`.`CON_ESTADO` AS `CON_ESTADO`,`c`.`VIP_COD` AS `VIP_COD`,`c`.`CON_FECHA` AS `CON_FECHA`,`c`.`GAR_COD` AS `GAR_COD`,`vs`.`VIS_COD` AS `VIS_COD`,`vs`.`VIS_NOMBRE` AS `VIS_NOMBRE`,`vs`.`VIS_APELLIDO` AS `VIS_APELLIDO`,`vs`.`PAR_COD` AS `PAR_COD`,`vs`.`VIS_CEDULA` AS `VIS_CEDULA`,`vs`.`VIS_DIRECCION` AS `VIS_DIRECCION`,`vs`.`VIS_TELEFONO` AS `VIS_TELEFONO`,`vs`.`VIS_CORREO` AS `VIS_CORREO`,`vs`.`VIS_IMAGEN` AS `VIS_IMAGEN`,`vs`.`VIS_ESTADO` AS `VIS_ESTADO`,`p`.`PPL_COD` AS `PPL_COD`,`p`.`CEL_COD` AS `CEL_COD`,`p`.`PPL_NOMBRE` AS `PPL_NOMBRE`,`p`.`PPL_APELLIDO` AS `PPL_APELLIDO`,`p`.`PPL_CEDULA` AS `PPL_CEDULA`,`p`.`PPL_IMG` AS `PPL_IMG`,`p`.`PPL_ESTADO` AS `PPL_ESTADO` from (((`bulkenco_sysecure`.`sys_visitante` `vs` join `bulkenco_sysecure`.`sys_visitante_ppl` `vp`) join `bulkenco_sysecure`.`sys_control` `c`) join `bulkenco_sysecure`.`sys_ppl` `p`) where ((`vs`.`VIS_COD` = `vp`.`VIS_COD`) and (`c`.`VIP_COD` = `vp`.`VIP_COD`) and (`p`.`PPL_COD` = `vp`.`PPL_COD`) and (`c`.`GAR_COD` = 2) and (`c`.`CON_ESTADO` = 'A') and (`vs`.`VIS_ESTADO` = 'A'));
