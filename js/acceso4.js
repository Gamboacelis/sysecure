...
//    $('#listaAcceso4').children('tbody').children('tr').addClass('terminarVisita');
    ...
    var intervalFuncionario = setInterval(function() {
        intervaloTranscurridoFuncionario();
    }, 1000);
});
function recargaFuncionarios4() {
    var myTable = $('#listaAccesoFuncionarios4').dataTable({
        "bDestroy": true,
        "bRetrieve": true,
        "bStateSave": true,
        "bPaginate": true,
        "bServerSide": true,
        "sAjaxSource": "includes/controles/acceso4/accesoFuncionarios4_dataTable.php",
        "oLanguage": {
            "sEmptyTable": "No hay datos disponibles en la tabla",
            "sInfo": "Existen _TOTAL_ registros en total, mostrando (_START_ a _END_)",
            "sInfoEmpty": "No hay entradas para mostrar",
...
function intervaloTranscurridoFuncionario(interval) {
    $('#listaAccesoFuncionarios4 >tbody >tr').each(function() {
        var cod = $(this).find("td").eq(0).children('input').val();
        $.ajax({
            url: "./includes/controles/acceso4/acceso4_model.php?opcion=intervaloFuncionario",
            type: 'post',
            data: {codigo: cod},
            success: function(numero) {
                var valor = Math.round(numero);
                $("#progresoTiempoFun" + cod).html(valor + ' %');
                $("#progresoTiempoFun" + cod).css({"width": (valor + " min")});
            }
        });
